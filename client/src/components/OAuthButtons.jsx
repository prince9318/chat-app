...
const OAuthButtons = () => {
  const handleOAuthLogin = (url, provider) => {
    if (!backendUrl) {
      toast.error(
        `${provider} login is not configured. Please set VITE_BACKEND_URL in your deployment environment.`,
      );
      return;
    }
  ...
  const handleGoogleLogin = () => {
    handleOAuthLogin(getGoogleAuthUrl(), "Google");
  };

  const handleGithubLogin = () => {
    handleOAuthLogin(getGithubAuthUrl(), "GitHub");
  };
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      <div className="flex items-center gap-3 my-1">
        ...
        <span className="text-xs text-[var(--text-muted)] uppercase tracking-wide shrink-0">
          or continue with
        </span>
        <span className="h-px flex-1 bg-[var(--border-subtle)]" />
      </div>
